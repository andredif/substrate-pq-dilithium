...
use dilithium_custom_lib::{Keypair, verify};

/// An identifier used to match public keys against dilithium3 keys
pub const CRYPTO_ID: CryptoTypeId = CryptoTypeId(*b"dlt3");
...
impl FromEntropy for Public {
	fn from_entropy(input: &mut impl codec::Input) -> Result<Self, codec::Error> {
		let mut result = Self([0u8; 1952]);
		input.read(&mut result.0[..])?;
		Ok(result)
	}
}

impl AsRef<[u8; 1952]> for Public {
	fn as_ref(&self) -> &[u8; 1952] {
		&self.0
	}
}
...
impl UncheckedFrom<[u8; 1952]> for Public {
	fn unchecked_from(x: [u8; 1952]) -> Self {
		Public::from_raw(x)
	}
}
...
/// A signature (a 3993-bit value).
#[cfg_attr(feature = "full_crypto", derive(Hash))]
#[derive(Encode, Decode, MaxEncodedLen, PassByInner, TypeInfo, PartialEq, Eq)]
pub struct Signature(pub [u8; 3293]);
...
impl From<Signature> for [u8; 3293] {
	fn from(v: Signature) -> [u8; 3293] {
		v.0
	}
}

impl AsRef<[u8; 3293]> for Signature {
	fn as_ref(&self) -> &[u8; 3293] {
		&self.0
	}
}
...
impl UncheckedFrom<[u8; 3293]> for Signature {
	fn unchecked_from(data: [u8; 3293]) -> Signature {
		Signature(data)
	}
}

impl Signature {
	/// A new instance from the given 3293-byte `data`.
	///
	/// NOTE: No checking goes on to ensure this is a real signature. Only use it if
	/// you are certain that the array actually is a signature. GIGO!
	pub fn from_raw(data: [u8; 3293]) -> Signature {
		Signature(data)
	}

	/// A new instance from the given slice that should be 3293 bytes long.
	///
	/// NOTE: No checking goes on to ensure this is a real signature. Only use it if
	/// you are certain that the array actually is a signature. GIGO!
...
impl Public {
	/// A new instance from the given 1952-byte `data`.
	///
	/// NOTE: No checking goes on to ensure this is a real public key. Only use it if
	/// you are certain that the array actually is a pubkey. GIGO!
	pub fn from_raw(data: [u8; 1952]) -> Self {
		Public(data)
	}


	/// Return a slice filled with raw data.
	pub fn as_array_ref(&self) -> &[u8; 1952] {
		self.as_ref()
	}
}

impl ByteArray for Public {
	const LEN: usize = 1952;
}
...
impl TraitPair for Pair {
	...
	fn verify<M: AsRef<[u8]>>(sig: &Self::Signature, message: M, public: &Self::Public) -> bool {
		verify(&sig.0, &message.as_ref(), &public);
		true
	}

	/// Return a vec filled with raw data.
	fn to_raw_vec(&self) -> Vec<u8> {
		self.0.public.to_vec()
	}
}
